...
public class DactDbConnection {
    ...
    public boolean InsertStudentsIntoNewCourse(String NewCourse_id, String OldCouse_id, ArrayList<String> sList, String regDate) {
        ...
        try {
            ...
            for (int i = 0; i < sList.size(); i++) {
                ...
                query = "Select * from `student-course` where course_id = '" + OldCouse_id + "' and s_id = '" + sList.get(i) + "'";
    ...
    public boolean DeactCourse(String OldCouse_id) {
        ...
        try {
            ...
            int x = pst.executeUpdate();
            if (x == 1) {
                query = "update `student-course` set status = 'DACT' where course_id = '" + OldCouse_id + "'";
                pst = connection.prepareStatement(query);
                x = pst.executeUpdate();
                if (x > 0) {
                    connection.commit();
                    isProcessSuccess = true;
                    JOptionPane.showMessageDialog(new JFrame(), "Course Successfully Deactivated");
                } else {
                    connection.rollback();
                    JOptionPane.showMessageDialog(new JFrame(), "Course can not deactivate,Since no Student Registered with the course");
                }

            } else {
                connection.rollback();
                JOptionPane.showMessageDialog(new JFrame(), "error occured when deactivating  Course..");
            }
