...
public class dbConnection {
    ...
    public String[] get_relavant_Course(String s_id) {
        ...
        try {
            ...
            query = "SELECT courseID,starttime,endtime FROM `extraclasses` WHERE STR_TO_DATE(date, '%m/%d/%Y')=CURDATE() and extraclasses.courseID in (select c.course_id from course c,`student-course` sc where sc.course_id = c.course_id and c.status = 'ACT' and sc.s_id = ?)";
            stmt = con.prepareStatement(query);
            stmt.setString(1, s_id);
            result = stmt.executeQuery();
            while (result.next()) {
    ...
    public HashMap<Integer, Object[]> getAllPaymentDetails(String s_id) throws Exception {
        int count = 1;
        ...
        try {
            d = new DataSource();
            con = d.getConnection();
            String query = "select payments.*,course.course_description from payments,course where course.course_id = payments.course_id and `s_id`=? and course.status = 'ACT' order by id desc";
            ...
            while (result.next()) {
                ...
                rowData[1] = result.getString("course_id");
                rowData[2] = result.getString("course_description");
                rowData[3] = result.getString("year") + "-" + result.getString("month");
                rowData[4] = result.getString("amount");
                rowData[5] = result.getString("id");
            ...
            cm.closeStatements(stmt);
            //------------------------log Start--------------------------------------------
            logger.error(CourseDbConnection.class + ".getAllPaymentDetails(" + s_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public boolean reverse_payment(String course_id, String s_id, String year, String month, double amount) throws Exception {
        ...
        try {
            ...
            String query = "delete from payments where s_id = ? and course_id = ? and month = ? and year = ?";
            ...
            if (out > 0) {
                query = "delete from payment_bill_details where s_id = ? and course_id = ? and payment_month = ? ";
                ...
                int out1 = stmt.executeUpdate();

                if (out1 >0) {
                    query = "update payments_activation_details set status = 'PENDING' where course_id = ? and payment_month = ? and payment_year = ? and s_id = ?";

                    stmt = con.prepareStatement(query);
                    stmt.setString(1, course_id);
                    stmt.setString(2, month);
                    stmt.setString(3, year);
                    stmt.setString(4, s_id);

                    int out2 = stmt.executeUpdate();

                    if (out2 > 0) {
                        boolean x = insert_to_reversed_payments(course_id, s_id, year + "-" + month, amount);
                        if (x) {
                            con.commit();
                            success = true;
                        } else {
            ...
            cm.closeStatements(stmt);
            //------------------------log Start--------------------------------------------
            logger.error(CourseDbConnection.class + ".reverse_payment(" + s_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public boolean insert_to_reversed_payments(String course_id, String s_id, String year_month, double amount) throws SQLException, Exception {
        PreparedStatement stmt = null;
        ResultSet result = null;
        ...
        try {
            d = new DataSource();
            con = d.getConnection();
            String query = "INSERT INTO `reversed_payments`(`course_id`, `s_id`, `year_month`, `amount`) VALUES "
                    + " (?,?,?,?)";

            stmt = con.prepareStatement(query);
            stmt.setString(1, course_id);
            stmt.setString(2, s_id);
            stmt.setString(3, year_month);
    ...
    public HashMap<String, Integer> rejectPayments(paymentBean pb, Connection con) throws Exception {
        PreparedStatement stmt = null;
        ResultSet result = null;
        ...
        try {
            ...
            for (int i = 0; i < months.size(); i++) {
                ...
                String query = "update payments_activation_details set status = 'REJECT',comment = ? where course_id=? and s_id=? and payment_month = ? and payment_year=? ";
                stmt = con.prepareStatement(query);
                stmt.setString(1, pb.getComment());
            ...
            JOptionPane.showMessageDialog(new JFrame(), SucCount + " Payments Reject Successfully");
    ...
    public Boolean check_nocard_Students(String s_id) {
        PreparedStatement stmt = null;
        ResultSet result = null;
        DataSource d = null;
        Connection con = null;
        ...
        try {
            ...
            String query = "select *  FROM  `deact_students_no_cards`  WHERE  s_id = ?";
            stmt = con.prepareStatement(query);
            stmt.setString(1, s_id);
            result = stmt.executeQuery();
           if (result.next()) {
    ...
    public String[] chackSMSPaymentDetails(String sid) throws Exception {
        boolean count = false;
        PreparedStatement pst = null;
        ResultSet rs = null;
        DataSource d = null;
        ...
        System.out.println(dateattendence2);
        query = "select * from `sms_deact_students` where  `s_id` =? and status = 'ACT' ";
        ...
        try {
            ...
            rs = pst.executeQuery();
            if (!rs.isBeforeFirst()) {
                query = "select * from `sms_payments` where `s_id` =? and record_status = 'ACT' ";
                pst = connection.prepareStatement(query);
                pst.setString(1, sid);
                rs = pst.executeQuery();
                while (rs.next()) {
                    output[0] = "Active";
                    output[1] = rs.getString("payment_status");
                    output[2] = rs.getString("expire_month_year");
                }
            } else {
    ...
    public boolean makeSMSPayment(String sid) throws Exception {
        int count = 0;
        PreparedStatement pst = null;
        ResultSet rs = null;
        DataSource d = null;
        ...
        query = "update `sms_payments` set payment_status = 'COMPLETED',expire_month_year=?,payment_date = now() where `s_id` =? and record_status = 'ACT' ";

        try {
            d = new DataSource();
            connection = d.getConnection();
            pst = connection.prepareStatement(query);
            pst.setString(1, curMonth + "-" + (Integer.parseInt(curYear) + 1));
            pst.setString(2, sid);
            ...
            cm.closeStatements(pst);
            //------------------------log Start--------------------------------------------
            logger.error(dbConnection.class + ".makeSMSPayment() ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(connection);
    ...
    public int insert_additional_SMS(String body, String mobile, String s_id) {
        ...
        try {
            d = new DataSource();
            connection = d.getConnection();
            query = "insert into additional_sms_details (s_id,body,mobile,created_date,paid) values (?,?,?,curdate(),0) ";
    ...
    public int additional_SMS_cost() {
        ...
        try {
            d = new DataSource();
            connection = d.getConnection();
            query = "select count(*) cost from additional_sms_details where paid = 0 ";
    ...
    public int get_weekNumber() throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_sid_from_card_number( ) ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public String get_SNameFromID(String ID) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_sid_from_card_number(" + S_name + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public String get_stdnt_SMS_contact(String ID) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_stdnt_SMS_contact() ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public boolean check_SMS_eligibility(String ID) throws SQLException {
        String contact = null;
        PreparedStatement stmt = null;
        ResultSet result = null;
        DataSource d = null;
        ...
        try {
            d = new DataSource();
            con = d.getConnection();
            String query = "select s_id from sms_deact_students where s_id = ? and status = 'ACT'";
            stmt = con.prepareStatement(query);
            stmt.setString(1, ID);
            result = stmt.executeQuery();
            while (result.next()) {
    ...
    public boolean makePayment(paymentBean pb, Connection con) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < months.size(); i++) {
                ...
                if (!isInsert) {
                    query = "INSERT INTO `payments`( `year`, `month`, `date`, `course_id`, `s_id`, `amount`,`payment_issue_date`,`payment_issue_month`,`payment_issue_year`,card_type,comment) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public HashMap<Integer, Object[]> get_given_class_due_payments(String last_Pmonth, int last_Pyear, String curMonth, int curYear, HashMap<Integer, String> monthMap, boolean isFirstMonth) throws SQLException {
        ...
        try {
            ...
            //------------------------log Start--------------------------------------------
            logger.error(dbConnection.class
                    + ".get_given_class_due_payments ->error" + ex);
            //-------------------------log End---------------------------------------------
        } finally {
    ...
    public String getCourseRegDate(String s_id, String course_id) {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".getCourseRegDate(" + s_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public SelectedCourseStudentDetails get_student_details(String s_id, SelectedCourseStudentDetails temp) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_student_details(" + s_id + "," + temp + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public String get_sid_from_card_number(String cardNumber) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_sid_from_card_number(" + cardNumber + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public SelectedCourseStudentDetails get_lecturer_Course_details(String course_id, String s_id, SelectedCourseStudentDetails temp) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_lecturer_Course_details(" + course_id + "," + s_id + "," + temp + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public ArrayList<String> get_attendance_history(String s_id, String c_id) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_attendance_history(" + s_id + "," + c_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public boolean insert_to_pending_SMS() throws SQLException {
        ArrayList<String> attandance_history = new ArrayList<String>();
        PreparedStatement stmt = null;
        ResultSet result = null;
        DataSource d = null;
        ...
        try {
            ...
            con = d.getConnection();
            con.setAutoCommit(false);
            String query = "select distinct s_id from `student-course` where status = 'ACT' and s_id not in (select s_id from sms_payments where month_year = '" + a + "')";
            stmt = con.prepareStatement(query);
            result = stmt.executeQuery();
            while (result.next()) {
                String s_id = result.getString("s_id");
                Double amount = courseCon.calculate_SMS_fee(s_id);
                String query2 = "INSERT INTO `sms_payments`(`month_year`, `s_id`, `amount`, `status`) VALUES (?,?,?,?)";
                stmt = con.prepareStatement(query2);
                stmt.setString(1, curMonth + "-" + curYear);
                stmt.setString(2, s_id);
                stmt.setDouble(3, amount);
                stmt.setString(4, "PENDING");
                tot = stmt.executeUpdate();
                if (tot != 1) {
                    break;
                } else {
                    status = true;
            ...
            logger
                    .error(dbConnection.class
                            + ".insert_to_pending_SMS() ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public int insert_to_pending_SMS_for_all_year(String s_id, Connection con) throws SQLException {
        ...
        try {
            ...
            Double amount = courseCon.calculate_SMS_fee_for_year(s_id);
            String query2 = "update `sms_payments` set `month_year` =? , `amount`=? ,`status`=? where s_id = ?";
            stmt = con.prepareStatement(query2);
            stmt.setString(1, "ALL-" + curYear);
    ...
    public int get_total_pending_SMS_amount(String s_id) throws SQLException {
        ArrayList<String> attandance_history = new ArrayList<String>();
        PreparedStatement stmt = null;
        ResultSet result = null;
        DataSource d = null;
        ...
        try {
            d = new DataSource();
            con = d.getConnection();
            String query = "select sum(amount)tot from sms_payments  where s_id = ? and status = 'PENDING'";
            stmt = con.prepareStatement(query);
            stmt.setString(1, s_id);
            result = stmt.executeQuery();
            while (result.next()) {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_attendance_history(" + s_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public int update_pending_SMS(String s_id, Connection con) throws SQLException {
        ArrayList<String> attandance_history = new ArrayList<String>();
        PreparedStatement stmt = null;
        ResultSet result = null;
        DataSource d = null;
        ...
        try {
            ...
            String query = "update sms_payments set status = 'COMPLETED' where s_id = ? and status = 'PENDING'";
            stmt = con.prepareStatement(query);
            stmt.setString(1, s_id);
            tot = stmt.executeUpdate();
            ...
            logger
                    .error(dbConnection.class
                            + ".update_pending_SMS(" + s_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
    ...
    public ArrayList<String> getPaymentDetails(String courseId, String studntID, int noOfMonths) throws Exception {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".getPaymentDetails(" + courseId + "," + studntID + "," + noOfMonths + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
            cm.closeConnection(con);
    ...
    public HashMap<Integer, Object[]> get_all_class_payment_history(String s_id) throws SQLException {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_all_class_payment_history(" + s_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
        } finally {
            cm.closeConnection(con);
            cm.closeResult(result);
    ...
    public void printBill(String billID, Double amount) {
        HashMap a = new HashMap();
        a.put("bill_id", billID);
        a.put("SMS_Charge", "SMS Charges :" + amount);
    ...
    public String insert_to_bill_details(ArrayList<paymentBean> paymentArray, Connection con, String IssuedBy) throws SQLException, Exception {
        ...
        try {
            ...
            logger
                    .error(dbConnection.class
                            + ".get_attendance_history(" + bill_id + ") ->error" + ex);
            //-------------------------log End---------------------------------------------
            throw ex;
        } finally {
