...
public class UserDbConnection {
    ...
    public boolean updateUserRole(String userName, String UserRole) throws SQLException, Exception {
        ...
        public  int deactivated_noCard_students() throws Exception {
        PreparedStatement stmt = null;
        PreparedStatement stmt2 = null;
        ResultSet rs = null;
        Connection conn = null;
        int count = 0;
        int cardlength = configs.card_number_length;
        try {
            ...
            // select * students which do not have card numbers and date different between regdate and current date > 35
            str = "select tbl.* from (select s_id,s_YOR,CURDATE() curdate,card_number,TIMESTAMPDIFF(DAY, STR_TO_DATE(s_yor,'%d-%m-%Y'),CURDATE() ) diff from student where (length(card_number)<> "+configs.card_number_length+" or card_number IS NULL or card_number='') order by  STR_TO_DATE(s_yor,'%d-%m-%Y') desc)tbl where tbl.diff>35";
            stmt = conn.prepareStatement(str);
            rs = stmt.executeQuery(str);
            
            while (rs.next()) {
                
                String query = "INSERT INTO deact_students_no_cards values (?,?,?,?,?)";
                stmt2 = conn.prepareStatement(query);
                stmt2.setString(1, rs.getString("s_id"));
                stmt2.setString(2, rs.getString("s_YOR"));
                stmt2.setString(3, rs.getString("curdate"));
                stmt2.setString(4, rs.getString("card_number"));
                stmt2.setInt(5, rs.getInt("diff"));
                stmt2.executeUpdate();
                count++;
            }
         } catch (Exception ex) {
